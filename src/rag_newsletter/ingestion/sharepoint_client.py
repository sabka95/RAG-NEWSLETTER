...
class GraphTokenProvider:
    ...
    def get(self) -> str:
        if not self._tok or time.time() > self._exp - 60:
            ...
            res = self.app.acquire_token_for_client(
...
class GraphClient:
    def __init__(self, tp: GraphTokenProvider):
        self.tp = tp
        self.s = requests.Session()

    def get_json(self, url: str) -> dict:
        r = self.s.get(
            url, headers={"Authorization": f"Bearer {self.tp.get()}"}, timeout=30
        )
        r.raise_for_status()
        return r.json()
    ...
    def stream(self, url: str):
        r = self.s.get(
            url,
            headers={"Authorization": f"Bearer {self.tp.get()}"},
            timeout=60,
            stream=True,
        )
        r.raise_for_status()
...
class SharePointClient:
    ...
    def find_drive_id(self, name: str) -> Optional[str]:
        for d in self.list_drives():
            if d.get("name", "").casefold() == name.casefold():
                return d["id"]
        return None
    ...
    def list_files(
        ...
        url = (
            ...
            if not folder_id
            else f"{GRAPH}/drives/{drive_id}/items/{folder_id}/children"
        )
    ...
    def download(self, drive_id: str, item_id: str, dest_path: str) -> str:
        ...
        with (
            self.g.stream(f"{GRAPH}/drives/{drive_id}/items/{item_id}/content") as r,
    ...
    def download_multiple(
        ...
        logger.info(
            f"Download completed: {len(downloaded_files)} files to {output_path}"
        )
