...
class OptimizedRAGIngestionService:
    ...
    def ingest_documents(self, 
        ...
        logger.info(f"⚙️  Configuration: MCDSE-2B + HNSW + Binary Quantization")
        ...
        for batch_start in range(0, len(file_paths), batch_size):
            ...
            logger.info(f"📦 Traitement du lot {batch_start//batch_size + 1}: fichiers {batch_start+1}-{batch_end}")
            ...
            for file_path, documents in batch_results.items():
                if documents:
                    ...
                    logger.info(f"✅ {Path(file_path).name}: {len(documents)} documents")
                else:
        ...
        try:
            ...
            logger.info(f"💾 Ajout de {len(all_chunks)} chunks au vector store optimisé...")
            logger.info("⏳ Génération des embeddings MCDSE... (optimisé pour Apple Silicon)")
            
            ids = self.vector_store.add_documents(all_chunks)
            
            logger.info(f"✅ Embeddings générés et stockés: {len(ids)} vecteurs optimisés")
            
            # Statistiques détaillées
            stats = self.document_processor.get_processing_stats({
                fp: [doc for doc in all_chunks if doc.metadata.get('source_file') == Path(fp).name]
            ...
            logger.info(f"🎉 Ingestion optimisée terminée avec succès!")
    ...
    def search_with_document_filter(self, 
                                   query: str, 
    ...
    def compare_documents(self, 
                         ...
                         k_per_doc: int = 3) -> Dict[str, List[Dict]]:
        """
        ...
        try:
            ...
            for doc1, doc2 in document_pairs:
                ...
                comparison_results[f"{doc1}_vs_{doc2}"] = {
                    ...
                    doc2: results_doc2,
                    "query": query,
                    "similarities": self._analyze_similarities(results_doc1, results_doc2),
                    "differences": self._analyze_differences(results_doc1, results_doc2),
                }
            
            logger.info(f"✅ Comparaison terminée: {len(comparison_results)} paires analysées")
    ...
    def _analyze_similarities(self, results1: List[Dict], results2: List[Dict]) -> List[str]:
    ...
    def _analyze_differences(self, results1: List[Dict], results2: List[Dict]) -> List[str]:
        ...
        if results1 and results2:
            ...
            if abs(max_score1 - max_score2) > 0.2:
                differences.append(f"Différence significative de pertinence: {max_score1:.3f} vs {max_score2:.3f}")
